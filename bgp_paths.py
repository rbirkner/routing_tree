...
def build_paths(name, topo, originator_ases, ribs, output_path, testing):
    """
    ...
    for originator in originator_ases:
        ...
        i += 1
        print 'Process %d: %d of %d' % (name, i, len(originator_ases))
                ...
                if topo.has_node(node.asn):
                    ...
                    for neighbor in neighbors:
                        ...
                        if neighbor_asn not in path:
                            if edge_data['relationship'] == -1 and edge_data['as2'] == node.asn:
        ...
        if i % div == 0 or i == len(originator_ases):
            if not testing:
                with open('%spaths-%s.log' % (output_path, str(name)), 'a', 102400) as output:
                    for asn, data in paths.iteritems():
                        if asn in ribs:
                            for origin, path in data.iteritems():
                                output.write('%d|%d|%s\n' % (asn, origin, ','.join(str(w) for w in reversed(path))))
                paths = defaultdict(dict)
    if testing:
        return paths
...
def test():
    ...
    tests = [
        {
            ...
            'paths': {
                ...
                2: {3: [3], 7: [7, 4], 10: [10, 6, 3]},
                3: {3: [], 7: [7, 4, 2], 10: [10, 6]},
                4: {3: [3, 2], 7: [7], 10: [10, 6, 3, 2]},
                5: {3: [3], 7: [7, 4], 10: [10, 6, 3]},
    ...
    for test_case in tests:
        ...
        print '--> Built Topology with %d nodes and %d edges' % (topo.number_of_nodes(), topo.number_of_edges())
            ...
            for origin, path in paths.iteritems():
                if path:
                    if result[node][origin] != path:
                        print 'Error: Path to %d of node %d is %s should be %s' % \
                              (origin, node, str(result[node][origin]), str(path))
                        error = True
        if error:
            print '-> Test failed\n'
        else:
...
def main(argv):
    ...
    # start routing tree algorithm using MPI
        ...
        rib_asns = list()
        if argv.rib_asns != '-1':
            with open(argv.rib_asns) as infile:
                data = infile.read()
                rib_asns = json.loads(data)
        else:
            rib_asns = as_topo.nodes()

        print 'Start process %d of %d working on origins from %d to %d' % (rank, size, start, stop)
        build_paths(rank, as_topo, chunk, rib_asns, argv.outpath, False)

''' main '''
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('rank', help='rank of script - starts with 0')
    parser.add_argument('size', help='total number of scripts')
    ...
    parser.add_argument('rib_asns', help='file containing the asns of which we want to store the paths')
